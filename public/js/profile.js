  ...
  showProfile = () => {
    let profile = {
      ...
      success: function (res) {
        if ((res.status = 1)) {
          res.user_details[0]["first_name"]
            ? $("#fname").html(res.user_details[0]["first_name"])
            : false;
          res.user_details[0]["last_name"]
            ? $("#lname").html(res.user_details[0]["last_name"])
            : false;
          res.user_details[0]["email"]
            ? $("#email").html(res.user_details[0]["email"])
            : false;
          res.user_details[0]["dob"]
            ? $("#dob").html(res.user_details[0]["dob"])
            : false;
          res.user_details[0]["gender"]
            ? $("#gender").html(res.user_details[0]["gender"])
            : false;
          res.user_details[0]["mobile"] != 0
            ? $("#mobile").html(res.user_details[0]["mobile"])
            : false;
          res.user_details[0]["address"]
            ? $("#address").html(res.user_details[0]["address"])
            : false;
          res.user_details[0]["profile_photo"]
            ? $("#profile-photo").attr(
                "src",
                BASE_URL +
                  "/uploads/user_images/" +
          ...
          if (res.user_details[0]["gender"])
            $(
              "input[name=gender][value=" + res.user_details[0]["gender"] + "]"
            ).prop("checked", true);
