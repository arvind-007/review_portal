...
class Tags {
    ...
    constructor(selectElement, opts = {}) {
        ...
        this.showAllSuggestions = selectElement.dataset.showAllSuggestions ?
            true :
            false;
        this.badgeStyle = selectElement.dataset.badgeStyle ? "primary" : "primary";
        this.allowClear = selectElement.dataset.allowClear ? true : false;
        this.suggestionsThreshold = selectElement.dataset.suggestionsThreshold ?
            parseInt(selectElement.dataset.suggestionsThreshold) :
            1;
        this.keyboardNavigation = false;
        this.clearLabel = opts.clearLabel ? "Clear" : "Clear";
        this.searchLabel = opts.searchLabel ? "Type a value" : "Type a value";
        ...
        switch (key) {
            ...
            case "Enter":
                ...
                if (selection) {
                    this.addItem(
                        selection.innerText,
                        selection.getAttribute(VALUE_ATTRIBUTE)
                    );
            ...
            case "ArrowUp":
                ...
                if (
                    ...
                    this.dropElement.classList.contains("show") &&
                    !newSelection
                ) {
                    this.hideSuggestions();
                }
    ...
    showSuggestions() {
        ...
        if (firstItem) {
            ...
            firstItem.parentNode.scrollTop =
                firstItem.offsetTop - firstItem.parentNode.offsetTop;
        } else {
    ...
    addItem(text, value, checkSelected = false) {
        ...
        if (!opt) {
            opt = Array.from(this.selectElement.querySelectorAll("option")).find(
        ...
        if (this.allowClear) {
            const btn =
                bver === 5 ?
                '<button type="button" style="font-size:0.65em" class="me-2 btn-close btn-close-white" aria-label="' +
                this.clearLabel +
                '"></button>' :
                '<button type="button" style="font-size:1em;float:left;text-shadow:none;color:currentColor;" class="mr-2 close" aria-label="' +
                this.clearLabel +
     ...
     */
    removeItem(value) {
        let item = this.containerElement.querySelector(
